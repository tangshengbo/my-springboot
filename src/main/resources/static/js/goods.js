...
var vm = new Vue({
    ...
    methods: {
        ...
        //刷新列表
        reloadList() {
            console.log("totalPage:" + this.pageConf.totalPage + ",pageSize:" + this.pageConf.pageSize + ",:pageNum:" + this.pageConf.pageNum);
            this.search(this.pageConf.pageNum, this.pageConf.pageSize);
        },
        //条件查询
        search(pageNum, pageSize) {
            this.loadings();
            this.$http.post('/goods/findByConPage?pageSize=' + pageSize + '&pageNum=' + pageNum, this.searchEntity).then(result => {
                console.log(result);
                if (result.body.success) {
                    const data = result.body.data;
                    this.goods = data.rows;
                    this.pageConf.totalPage = data.total;
        ...
        handleCurrentChange(val) {
            this.pageConf.pageNum = val; //为了保证刷新列表后页面还是在当前页，而不是跳转到第一页
            ...
            this.$confirm('你确定永久删除此用户信息？', '提示', {
                ...
                this.$http.post('/goods/delete', JSON.stringify(ids)).then(result => {
                    if (result.body.success) {
                        ...
                        //刷新列表
                        //为什么要判断并赋值？
                        //答：即使调用reloadList()刷新列表，但是对于删除，在reloadList()中获取到的totalPage总记录和pageNum当前页都是未删除之前的记录，当遇到删除此页的最后一个记录时，页码会自动跳到上一页，但是table中的数据显示"暂无记录"
                        //   所以要判断，如果是删除此页的最后一条记录，删除后自动跳转到前一页，数据也是前一页的数据
                        if ((this.pageConf.totalPage - 1) / this.pageConf.pageSize === (this.pageConf.pageNum - 1)) {
